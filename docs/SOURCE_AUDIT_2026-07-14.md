...
- `amends`、`amended_by`、`repeals`、`supersedes`、`interprets`、`paired_with`；
...
它早于当前 `deeplaw.sqlite/v3` provenance schema，没有记录 OCR 渲染器版本、DPI、语言、
page segmentation mode 或独立 extracted-text hash，也没有坐标/置信度。当前代码会在新构建
中记录 Tesseract 与 `pdftoppm` 版本、上述 OCR 配置和 extracted-text hash，但不会追写旧
release，且仍未保留坐标/置信度。因此本节不能作为当前源码已通过 runtime 或 G2 门禁的
证据；必须重新构建、复核并产生新的证据后才能更新现状声明。

### 2026-07-15 当前 v3 本地候选
...
扫描件当前记录 `tesseract 5.5.2; pdftoppm version 24.02.0`、300 DPI、
...
| 路径、大小和 hash 完整性 | 通过 |
| 文件容器基础安全 | 通过 |
| 全部正文可检索（当前候选） | v3 候选构建通过；1 份 OCR 派生仍需逐页人工复核 |
| 法律时效和版本关系 | 未通过，需结构化复核 |
| 案例隐私 | 未通过，需人工复核 |
...
- [ ] 将 manifest 升级为 DeepLaw 法律版本 schema；
- [ ] 补齐 28 份文档的发布机关、文号、效力状态和版本关系；
- [x] 对纯扫描 PDF 执行一次本地 OCR，并保留当时的 Tesseract 版本和人工复核标记；
- [x] 用当前 schema 重建候选，记录 Tesseract/Poppler 版本、OCR 配置和 extracted-text hash；
...
- 未对全部页面进行视觉渲染比对；
- 未完成正式法律时效、许可或隐私意见；
- 已对纯扫描 PDF 做本地 OCR 和首页视觉检查，但未完成 8 页逐字复核；
- 2026-07-15 的候选构建早于当前 storage/provenance schema，尚未用当前源码重新构建验证；
