...
| MCP read surface | default `query_plan_version=6`; recommended `query/context/wiki/source/verify` | Explicit v5 MCP output/v3 with Capsule v2 and Query Plan v5; scheduled `search`/`recall`/`wiki_lookup` routes remain | `knowledge_support` stays read-only; v6 query/context unstructured MCP `content` is the exact canonical nested Provider Capsule counted by `provider_content_bytes`, while the validated full result remains `structuredContent`; Provider text is ≤64 KiB and never carries the outer receipt/delivery wrapper, full plan, candidate scores, rejected-candidate text, or local audit internals |
...
- Tolaria targets `v2026-08-11`, annotated-tag commit
  `cb45f26649a7500e0bdb5dd0b8f0412e9c1daf4d` (package `0.1.0`). Its external MCP boundary is supported, while the
