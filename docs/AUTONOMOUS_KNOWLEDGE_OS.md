...
revision. The current autonomous `forget` operation removes recall, workspace, and derived-index
eligibility while retaining immutable revision/audit history. Content-erasing owner GC for
historical Agent revisions and general Knowledge OS user-source snapshots is **Planned**, not a
delivered implication of `forget` or legacy source tombstoning; user-private Legal Pack deletion
remains a separate existing owner operation.
...
The additive STRICT tables in `vault.sqlite3` record:
...
- sink grants, idempotency, usage, and feedback;
...
FTS, graph adjacency, deterministic connected-component communities, Living Wiki navigation,
semantic-lint reports, JSON Canvas, and query caches are disposable. Their manifest binds the
...
The current default implementation is deterministic lexical/graph navigation. Model-backed
embedding, reranking, entity extraction, Source Tree reasoning, and GraphRAG summaries remain
optional or planned until their exact model/configuration/data-egress contract and evaluation are
bound. They are not described as default production capability.

## 3. Autonomous mutation lifecycle

The `knowledge_sink` domain path applies this sequence:

1. **Capture** only explicit durable content; raw conversation and hidden chain-of-thought are not
   accepted as a persistence instruction.
2. **Classify** kind, exact scope, maximum sensitivity, epistemic state, temporal interval, TTL,
   source binding, and generation activity.
...
4. **Reconcile** idempotency and compare-and-swap. Exact same-scope semantic duplicates are
   rejected with the existing stable ID. Contradictions remain independent objects/relations.
...
8. **Connect and rebuild** FTS, canonical relations, lint, Wiki, communities, and Canvas.
...
10. **Decay or forget** through TTL expiry or an explicit immutable lifecycle revision. Current
    workspace and indexes stop exposing inactive content; history and audit identities remain.
...
`authority=agent_derived`, and `legal_authority=false`. Ranking, link count, community membership,
feedback, model confidence, or repeated retrieval cannot change these fields.
...
`recall` currently discovers exact IDs and lexical candidates, then admits by lifecycle, exact
scope, maximum sensitivity, kind, TTL, valid time, and transaction time. Selection applies item and
character/provider-payload budgets. Unauthorized scope or sensitivity candidates are omitted
without identifiers or counts; admitted candidates can still expose bounded rejection reasons.
The v2 `kinds` filter is the explicit union of legacy Asset kinds and autonomous Knowledge kinds;
...
statement that ranking did not change authority.
...
It also binds the autonomous kind filter. The source-derived partition emits its own hashed compact
...
Before either v1 or v2 `knowledge_support` response leaves the process, a recursive projection
...
- v2 contract after autonomous migration, v1 compatibility contract for untouched v0.7 Vaults;
- search/recall, get, context, verify, inspect, lineage, graph, and Wiki lookup;
...
Creating the autonomous core does not create a sink grant. The default Knowledge OS plugin only
...
`save_skill` stores a Skill as a versioned Agent-derived Knowledge Object. Its closed manifest
