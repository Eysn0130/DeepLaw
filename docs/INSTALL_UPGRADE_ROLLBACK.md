# DeepLaw v0.12.0 installation, upgrade, and rollback
...
[`v0.12.0` GitHub Release](https://github.com/Eysn0130/DeepLaw/releases/tag/v0.12.0).

## Verify and install

Download `deeplaw-0.12.0-py3-none-any.whl`, `SHA256SUMS`, and the wheel's
...
On macOS, compare `shasum -a 256 deeplaw-0.12.0-py3-none-any.whl` with its line in
`SHA256SUMS`. On Windows PowerShell:

```powershell
Get-FileHash .\deeplaw-0.12.0-py3-none-any.whl -Algorithm SHA256
```
...
python -m sigstore verify identity \
  --bundle deeplaw-0.12.0-py3-none-any.whl.sigstore.json \
  --cert-identity \
    https://github.com/Eysn0130/DeepLaw/.github/workflows/release.yml@refs/tags/v0.12.0 \
  --cert-oidc-issuer https://token.actions.githubusercontent.com \
  deeplaw-0.12.0-py3-none-any.whl
```

Verify GitHub provenance when `gh` is available:

```bash
gh attestation verify deeplaw-0.12.0-py3-none-any.whl --repo Eysn0130/DeepLaw
```

Install only the verified bytes:

```bash
uv tool install ./deeplaw-0.12.0-py3-none-any.whl
deeplaw --version
deeplaw doctor
```

The expected version output is `deeplaw 0.12.0`.

## Create a new v0.12 Vault
...
Initialization creates both the retained source-derived compatibility tables and the v0.12
autonomous Markdown/Ledger, Semantic Compilation v2, and Synthesis Refresh cores. It does not
create a mutation grant.
...
v0.12.0 adds Semantic Compilation v2, run-wide finalization, and Synthesis Refresh to the existing
v0.11 governed compiler. Stop every CLI, Watcher, MCP, and editor integration, then create and
verify a rollback snapshot before replacing the application:

```bash
deeplaw knowledge snapshot create \
  --vault ./vault --output ./snapshot-before-v0.12
deeplaw knowledge snapshot verify --snapshot ./snapshot-before-v0.12

uv tool install --upgrade ./deeplaw-0.12.0-py3-none-any.whl
deeplaw --version
deeplaw knowledge autonomy migrate \
  --vault ./vault --backup ./pre-v0.12-semantic-backup
deeplaw knowledge doctor --vault ./vault --permissions
deeplaw knowledge autonomy verify --vault ./vault
deeplaw knowledge autonomy rebuild --vault ./vault
```

Migration retains the existing Source/Knowledge identity and audit chains, installs additive
semantic inventory, duty, finalization, and refresh state within the existing
`deeplaw.source-compilation-core/v1` migration identity, and records the exact migration
implementation in the release manifest. It does not reinterpret or re-import source bytes.

For application rollback, stop all processes, restore `snapshot-before-v0.12` to a separate path,
verify it, atomically replace the active Vault using the documented snapshot restore procedure, and
then reinstall the verified v0.11.0 wheel. Never run both versions against one Vault or point the
...
Install v0.12.0, then create an explicit verified rollback point and install the additive autonomous
core:
...
Keep both `snapshot-before-v0.12` and `pre-autonomy-v0.7-backup` until real recall, MCP, integrity,
...
v0.12 autonomous migration. Do not skip either rollback point:
...
The release gate independently exercises a clean v0.6.0 wheel upgrade into the v0.12.0
distribution and verifies migration, rollback, snapshot, restore, uninstall, and reinstall.

## Roll back the autonomous migration

Rollback is explicit and keeps the replaced v0.12 Vault in a sibling recovery directory:
...
autonomous migration completed and later v0.12 writes were made, rollback intentionally removes
those writes from the active Vault; preserve the v0.12 recovery directory for audit or selective
re-entry.
...
deeplaw knowledge snapshot restore \
  --snapshot ./snapshot-before-v0.12 \
  --vault ./vault-restored-v0.7 \
  --confirm
```

Application rollback is separate from Vault rollback. Do not open a v0.12 autonomous Vault with an
...
## v0.12 snapshots and credentials
...
`deeplaw-0.12.0-linux-amd64.oci.tar` is an OCI layout archive, not a remotely listening service. Its
