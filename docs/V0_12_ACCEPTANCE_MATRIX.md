...
| 52 | Local verified + Formal gate — Linux/macOS/Windows exact commit; pull-request evidence checks out the exact head rather than the synthetic merge ref | `.github/workflows/ci.yml`, `.github/workflows/commercial-gates.yml` nine-cell matrix | `test_pull_request_gates_check_out_the_exact_head_commit`; exact-tag platform JSON/JUnit artifacts |
...
| 55 | Formal deterministic machine-consensus gate | `semantic-evidence.yml`, deterministic lifecycle/query suite, six isolated packet contracts | exact-commit lifecycle/query/cost, six unanimous packets, consensus, bilingual Owner packet, and manifest `formal_release_eligible=true`; external real-model execution remains `not_executed` and is not substituted by no-model evidence |
