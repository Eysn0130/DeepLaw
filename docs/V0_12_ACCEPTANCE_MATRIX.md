...
| 28 | Local verified — fresh Synthesis is compiled-first and single-target noise, including unrelated stale/uncompiled gaps and internal overfetch/relevance-floor rejections, is suppressed | Query Plan v5 admission/target-scoped selection, suppressed-candidate receipt, and bounded meaningful-term gap matching | `test_purpose_aware_query_is_compiled_first_and_read_only`; `test_uncompiled_source_gap_uses_all_meaningful_query_terms`; `test_v5_target_scoped_capsule_suppresses_internal_candidate_noise`; successor gap regression; frozen 15-case CLI suite |
