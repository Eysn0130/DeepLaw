...
| 28 | Local verified — fresh Synthesis is compiled-first and single-target noise, including unrelated stale/uncompiled gaps, is suppressed | Query Plan v5 admission/target-scoped selection and bounded meaningful-term gap matching | `test_purpose_aware_query_is_compiled_first_and_read_only`; `test_uncompiled_source_gap_uses_all_meaningful_query_terms`; successor gap regression; frozen 15-case CLI suite |
