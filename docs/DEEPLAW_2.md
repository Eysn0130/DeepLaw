...
当前软件版本是 `v0.4.0`。本文记录独立 Legal Pack；通用 Knowledge Asset 内核见
...
可追溯的 Knowledge Release，并向 Agent 交付小型 Evidence Pack；当前 `v0.4.0` 官方团队
...
官方目录、用户私有法律参考与 Analytix 案件项目必须物理隔离。当前 `v0.4.0` 不提供内容级
...
`v0.4.0` 已实现固定 release、integrity/抽取门禁、显式时点查询分桶、coverage-first 选择和
...
Evidence Pack 是 Agent 实际看到的有界结果。`v0.4.0` 已有 `query_plan`、
...
- 完整逐候选 execution trace、trace hash 与按 ID 读取接口属于后续目标，`v0.4.0` 不作此声明。
...
当前 `v0.4.0` 已实现 QueryPlan、封闭 Evidence Duties、相关性准入、coverage-first 选择、
...
`v0.4.0` 仍以显式 temporal classification、抽取标记和 compiler 的 uncertain boolean 投影其中一部分：
