...
| `P4-WIN-STABILITY` | Current-head Windows Python 3.11/3.12/3.13 reproduced `statement_count=0`; a redacted failure fingerprint bound it to `RSS child process returned no report (1)` | The RSS child used a second bespoke three-variable environment instead of the shared closed environment, omitting Windows bootstrap and isolated home variables; after the child was restored, Windows correctly reported current RSS as unsupported rather than attempting requests | Runtime-stability raw report and Candidate CI Windows preflight | The broad fixture catch misreported an RSS-child launch failure as an empty canonical fixture; treating unsupported Windows RSS as executed would also fabricate evidence | Reuse the closed environment helper, map the caller-owned isolated `HOME` to Windows `USERPROFILE`, keep Python flags as fixed non-secret constants, and assert `not_executed` for the unsupported Windows RSS measurement while still executing fixture and 8-reader checks | `tests/test_v013_runtime_stability.py`; `tests/test_subprocess_environment.py`; Windows preflight in `.github/workflows/ci.yml` | Public fixture now remains at one Statement; all three Windows minors pass the focused preflight before the full suite | The 10,000-request RSS Core lane remains `not_executed` and must run on a supported measurement platform |
| `P4-CI-PYTHON-RECEIPT` | Candidate receipt for matrix Python 3.11 reported the runner's system Python 3.12 even though pytest itself used uv-managed 3.11 | The receipt heredoc invoked ambient `python` instead of the exact uv environment | Candidate CI evidence receipt | A green matrix could carry false interpreter provenance | Generate the receipt with `uv run --frozen python` and fail closed when the actual major/minor differs from the matrix cell | Current-head Candidate CI receipts and workflow inspection | macOS 3.13 receipt proved exact 3.13.14; final nine-row current-head receipts pending | Candidate CI remains claim-ineligible development evidence, not Platform Core |
