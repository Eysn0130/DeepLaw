...
def test_ocr_mutations_are_deterministic_and_cannot_claim_review() -> None:
    ...
    assert {case["expected_capability"] for case in first["cases"]} == {
        "extraction:ocr_unreviewed"
    }
...
def test_internal_nonlegal_pack_core_is_verified_without_a_public_mcp_leaf() -> None:
    fixture = json.loads(
        (
            REPOSITORY
            / "benchmarks/legal/synthetic-policy-pack-core-v1.json"
        ).read_text(encoding="utf-8")
    )
    schema = json.loads(
        (
            REPOSITORY / "contracts/authoritative-pack-core.v1.schema.json"
        ).read_text(encoding="utf-8")
    )
